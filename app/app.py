...
def send_to_ollama(images_b64, prompt):
    ...
    try:
        resp = requests.post(OLLAMA_ENDPOINT, json=payload, timeout=60)  # timeout sec
...
def record_clip(out_path, camera_id=0, duration=5):
    ...
    size = (int(width), int(height))  # ensure ints
            ...
            try:
                ...
                try:
                    if isinstance(response, str):
                        # Safely parse JSON-like string response
                        import json
                        resp_dict = json.loads(response)  # safer than eval
                    else:
                        resp_dict = response
